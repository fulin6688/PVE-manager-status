...
if ! command -v sensors > /dev/null; then
    ...
    if apt update && apt install -y lm-sensors; then 
        ...
        echo "尝试继续安装 linux-cpupower 获取功耗信息"
        if apt install -y linux-cpupower;then
            echo "linux-cpupower 安装成功"
        else
            echo -e "linux-cpupower 安装失败，可能无法正常获取功耗信息，你可以使用\033[34mapt update ; apt install linux-cpupower && modprobe msr && echo msr > /etc/modules-load.d/turbostat-msr.conf && chmod +s /usr/sbin/turbostat && echo 成功！\033[0m 手动安装"
        fi
    else
        echo "脚本自动安装所需依赖失败"
...
如果想强制重新修改，请执行：\033[31m\"$sap\" remod\033[0m 命令
"
    exit 1
}
...
如果你看到主页面提示连接错误或者没看到温度和频率，请按：\033[31mShift+F5\033[0m
如果想还原，请执行：\033[31m\"$sap\" restore\033[0m
"
